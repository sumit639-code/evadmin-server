...
const ScooterData = () => {
  ...
  useEffect(() => {
    if (scooters.length > 0) {
      const uniqueModels = [...new Set(scooters.map(scooter => scooter.model))];
  ...
  const fetchScooters = async () => {
    ...
    try {
      ...
      if (selectedModel === "All Models") {
        const uniqueModels = [...new Set(response.data.scooters.map(scooter => scooter.model))];
        uniqueModels.sort(); // Sort models alphabetically
        setModelOptions(uniqueModels);
      }
    } catch (error) {
  ...
  const fetchMaintenanceLogs = async (scooterId) => {
    try {
      const response = await axiosInstance.get(`/scooter/${scooterId}/maintenance-logs`);
  ...
  const handleSave = async () => {
    try {
      ...
      // Update local state
      setScooters(scooters.map(s => s.id === selectedScooter.id ? response.data.scooter : s));
      setSelectedScooter(response.data.scooter);
      setEditMode(false);
  ...
  const handleSaveNewScooter = async () => {
    if (selectedScooter.model && selectedScooter.owner && selectedScooter.pricePerHour) {
      try {
        // Prepare data for API
        const scooterData = {
          model: selectedScooter.model,
  ...
  return (
    <div className="p-4">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {scooters.map((scooter) => (
                <tr key={scooter.id} onClick={() => handleRowClick(scooter)} className="cursor-pointer hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{scooter.scooterId}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{scooter.model}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{scooter.owner}</td>
      ...
      <div className="flex items-center justify-between mt-4">
        <div className="text-sm text-gray-700">Showing {scooters.length} scooters</div>
        <div className="flex items-center space-x-2">
          ...
          <div className="bg-white rounded-lg max-w-3xl w-full">
            <div className="p-6">
              <div className="flex justify-between items-start mb-6">
                <h2 className="text-2xl font-bold">{editMode ? (selectedScooter.id ? "Edit Scooter" : "Add New Scooter") : "Scooter Details"}</h2>
              ...
              <div className="grid gap-6">
                      ...
                      {editMode && selectedScooter.model === "new" && (
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setSelectedScooter({...selectedScooter, newModel: e.target.value})}
                    ...
                    <div className="border rounded-lg overflow-hidden">
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                          <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Model</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Last Maintained</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                          </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                          {maintenanceLogs.length > 0 ? (
                            maintenanceLogs.map((record, index) => (
                              <tr key={index}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{record.lastMaintained}</td>
                                <td className="px-6 py-4 whitespace-nowrap">
                ...
                <div className="flex justify-end">
                  {editMode ? (
                    <button 
                      onClick={selectedScooter.id ? handleSave : handleSaveNewScooter} 
